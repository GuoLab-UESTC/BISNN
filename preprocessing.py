...
DATA_DIR = "/home/sunhongze/Dataset/DEAP/data_preprocessed_python/"
...
def baseline_removal_cross(data):
    """
    calculate the baseline signal per second
    then subtract that baseline from the signal
    """
    ...
    for ins in range(data.shape[0]):
        ...
        for c in range(data.shape[1]):
            ...
            preprocessed_ins.append(signal_split)
        preprocessed.append(preprocessed_ins)
    # preprocessed = np.array(preprocessed).transpose(0, 2, 1, 3)
    # conv = list([])
    # for trial in preprocessed:
    #     conv.append(np.dot(trial,trial.T))
    # conv_ = np.array(conv).mean(0)
    # print()
    return np.array(preprocessed).transpose(0, 2, 1, 3)
...
def dataset_prepare(segment_duration = 3, n_subjects = 32, load_all = True, single_subject = False, sampling_rate = 128,label_type = [0,2],subject = 1):
    ...
    # train_x = torch.load('/home/sunhongze/PyProjects/ANN-SNN/BISNN/DEAP/predata/train_x.t7')
    # train_y = torch.load('/home/sunhongze/PyProjects/ANN-SNN/BISNN/DEAP/predata/train_y.t7')
    # test_x = torch.load('/home/sunhongze/PyProjects/ANN-SNN/BISNN/DEAP/predata/test_x.t7')
    # test_y = torch.load('/home/sunhongze/PyProjects/ANN-SNN/BISNN/DEAP/predata/test_y.t7')
    # train_dataset = TensorDataset(train_x, train_y.long()) # create your datset
    # test_dataset = TensorDataset(test_x, test_y.long())
    return train_dataset, test_dataset

def dataset_prepare_for_cross(segment_duration = 3, n_subjects = 32, load_all = True, sampling_rate = 128,label_type = [0,2], subjects_train = [0,1,2,3,4,5], subjects_test = [0,1,2,3,4,5]):
    x = []
    y = []
    for i in range(n_subjects):
        data = load_DEAP(DATA_DIR, n_subjects = i, single_subject=True,load_all = False,label_type=label_type)
        s1, s1_labels, s1_names = data
        s1_labels = np.repeat(s1_labels.reshape(-1, 1), 20)
        s1_preprocessed = baseline_removal_cross(s1)
        # s1_preprocessed = EA(s1_preprocessed)
        s1_preprocessed = standardize(s1_preprocessed)
        b, s, c, n = s1_preprocessed.shape
        s1_preprocessed = s1_preprocessed.reshape(b*s, c, -1)
        x.append(s1_preprocessed)
        y.append(s1_labels)

    # for i in range(32):
    #     print('=============================================')
    #     print('min is ', x[i].min())
    #     print('max is ', x[i].max())
    train_x = torch.Tensor(np.array([x[sub] for sub in subjects_train])).reshape(-1,32,segment_duration*sampling_rate).permute(0,2,1) # transform to torch tensor
    train_y = torch.Tensor(np.array([y[sub] for sub in subjects_train])).reshape(-1,1)
    test_x = torch.Tensor(np.array([x[sub] for sub in subjects_test])).reshape(-1,32,segment_duration*sampling_rate).permute(0,2,1)   # transform to torch tensor
    test_y = torch.Tensor(np.array([y[sub] for sub in subjects_test])).reshape(-1,1)
